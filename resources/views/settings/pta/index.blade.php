...
<a href="{{url('settings/satker')}}" class="ml-1 d-none d-sm-inline-block btn btn-sm btn-danger">
      ...
      <table id="tabel" class="display" style="width:100%">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>{{$d->alamat}}</td>
            <td class="d-flex">
              <form action="settings/pta/{{$d->id}}" method="post">
                @csrf
                @method('delete')
                <button class="btn btn-danger btn-sm btn-circle rounded"><i class="fa fa-trash"></i></button>
              </form>
              
              <a href="{{url('settings/pta/'.$d->id.'/edit')}}" class="btn btn-success btn-sm btn-circle rounded mx-1"><i class="fa fa-edit"></i></a>
              
              <a href="{{url('settings/pta/'.$d->id)}}" class="btn btn-primary btn-sm btn-circle rounded"><i class="fa fa-folder-open"></i></a>
            </td>
          </tr>
          @endforeach
        </tbody>
...
<div class="modal fade" id="modal" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <form action="{{url('settings/pta')}}" method="POST">
        @csrf
        <div class="modal-body">
          <div class="form-group">
            <label for="nama_pta">Nama PTA</label>
            <input type="text" class="form-control" name="nama_pta" id="nama_pta">
          </div>
          <div class="form-group">
